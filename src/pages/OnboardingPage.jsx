...
// ─── BUG FIX: These components MUST be defined OUTSIDE the parent component.
// When defined inside, React recreates them as new component types on every
// keystroke → input unmounts and remounts → focus is lost after each character.

function FormInput({ label, field, type = 'text', placeholder = '', hint = '', value, onChange, error }) {
  return (
    <div className="form-group">
      ...
      {hint && !error && <div style={{ fontSize: 11, color: 'var(--slate)' }}>{hint}</div>}
    </div>
  )
}

function FormSelect({ label, field, options, value, onChange, error }) {
  return (
    <div className="form-group">
      <label className="form-label">{label}</label>
      <select className="form-select" value={value} onChange={e => onChange(field, e.target.value)}>
...
export default function OnboardingPage({ navigate }) {
  ...
  // Stable handler — doesn't cause child component recreation
  ...
  if (submitted) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fafbfd' }}>
      <div className="card animate-fade" style={{ maxWidth: 480, width: '90%', textAlign: 'center', padding: 48 }}>
        <div style={{ width: 72, height: 72, background: 'var(--success-light)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px' }}>
        ...
        <div style={{ background: 'var(--surface)', borderRadius: 10, padding: '16px 20px', marginBottom: 28, textAlign: 'left' }}>
          ...
          <div style={{ fontFamily: 'DM Serif Display, serif', fontSize: 22, color: 'var(--ink)' }}>CF-{Date.now().toString().slice(-8)}</div>
        </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fafbfd', paddingBottom: 60 }}>
      <div style={{ background: 'white', borderBottom: '1px solid var(--border)', padding: '20px 48px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <button onClick={() => navigate('landing')} style={{ background: 'none', border: 'none', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
          <div style={{ width: 34, height: 34, background: 'var(--ink)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--gold)', fontSize: 16, fontFamily: 'DM Serif Display, serif' }}>C</div>
        ...
        <div className="card animate-fade">
          ...
          {step === 0 && <>
            ...
            <div className="grid-2">
              <FormInput label="GSTIN *" field="gstin" placeholder="22AAAAA0000A1Z5" hint="15-character GST Identification Number" value={form.gstin} onChange={handleChange} error={errors.gstin} />
              <FormInput label="Firm / Company Name *" field="firmName" placeholder="ABC Enterprises Pvt Ltd" value={form.firmName} onChange={handleChange} error={errors.firmName} />
            </div>
            <div className="grid-2">
              <FormSelect label="Firm Type *" field="firmType" options={['Sole Proprietorship','Partnership','LLP','Private Limited','Public Limited','HUF','Trust']} value={form.firmType} onChange={handleChange} error={errors.firmType} />
              <FormInput label="PAN *" field="pan" placeholder="AAAAA0000A" hint="10-character PAN of the entity" value={form.pan} onChange={handleChange} error={errors.pan} />
            </div>
            <div className="grid-2">
              <FormInput label="Year of Establishment" field="yearEstd" type="number" placeholder="2010" value={form.yearEstd} onChange={handleChange} error={errors.yearEstd} />
              <FormSelect label="Annual Turnover Range" field="turnoverRange" options={['< ₹50 Lakhs','₹50L – ₹2Cr','₹2Cr – ₹10Cr','₹10Cr – ₹50Cr','> ₹50Cr']} value={form.turnoverRange} onChange={handleChange} error={errors.turnoverRange} />
            </div>
            <FormSelect label="Industry / Sector" field="industry" options={['Manufacturing','Trading','Services','Agriculture & Allied','Construction','Hospitality','Healthcare','Education','Logistics','Technology','Other']} value={form.industry} onChange={handleChange} error={errors.industry} />
            ...
            <FormInput label="Address" field="address" placeholder="Plot No., Street, Area" value={form.address} onChange={handleChange} error={errors.address} />
            <div className="grid-3">
              <FormInput label="City" field="city" placeholder="Mumbai" value={form.city} onChange={handleChange} error={errors.city} />
              <FormSelect label="State" field="state" options={STATES} value={form.state} onChange={handleChange} error={errors.state} />
              <FormInput label="Pincode" field="pincode" placeholder="400001" value={form.pincode} onChange={handleChange} error={errors.pincode} />
            </div>
          </>}
          ...
          {step === 1 && <>
            ...
            <div className="grid-2">
              <FormInput label="Full Name *" field="propName" placeholder="As per PAN card" value={form.propName} onChange={handleChange} error={errors.propName} />
              <FormInput label="Date of Birth" field="propDob" type="date" value={form.propDob} onChange={handleChange} error={errors.propDob} />
            </div>
            <div className="grid-2">
              <FormInput label="PAN *" field="propPan" placeholder="AAAAA0000A" value={form.propPan} onChange={handleChange} error={errors.propPan} />
              <FormInput label="Aadhaar Number" field="propAadhaar" placeholder="XXXX XXXX XXXX" hint="Last 4 digits will be masked" value={form.propAadhaar} onChange={handleChange} error={errors.propAadhaar} />
            </div>
            <div className="grid-2">
              <FormInput label="Mobile Number *" field="propMobile" placeholder="9876543210" hint="For OTP and updates" value={form.propMobile} onChange={handleChange} error={errors.propMobile} />
              <FormInput label="Email Address *" field="propEmail" type="email" placeholder="name@company.com" value={form.propEmail} onChange={handleChange} error={errors.propEmail} />
            </div>
            <div className="divider" />
            <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--ink-muted)', marginBottom: 16 }}>Additional Director / Partner (if applicable)</div>
            <div className="grid-2">
              <FormInput label="Name" field="directorName2" placeholder="Second Director / Partner" value={form.directorName2} onChange={handleChange} error={errors.directorName2} />
              <FormInput label="PAN" field="directorPan2" placeholder="AAAAA0000A" value={form.directorPan2} onChange={handleChange} error={errors.directorPan2} />
            </div>
          </>}
          ...
          {step === 2 && <>
            ...
            <p style={{ color: 'var(--slate)', fontSize: 13, marginBottom: 24 }}>Accepted formats: PDF, JPG, PNG (max 10MB each)</p>
            <div style={{ background: 'var(--warning-light)', border: '1px solid #f0c56a', borderRadius: 10, padding: '12px 16px', marginBottom: 24, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--warning)' }}>Documents are encrypted in transit and at rest. Accessible only to our credit team.</div>
            </div>
            <div style={{ marginBottom: 8, fontSize: 13, fontWeight: 600, color: 'var(--ink-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>GST Documents</div>
            ...
            <div className="divider" />
            <div style={{ marginBottom: 8, fontSize: 13, fontWeight: 600, color: 'var(--ink-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Credit & Financial Documents</div>
            ...
            <div className="divider" />
            <div style={{ marginBottom: 8, fontSize: 13, fontWeight: 600, color: 'var(--ink-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Identity & Business Proof</div>
          ...
          {step === 3 && <>
            ...
            <FormInput label="Username *" field="username" placeholder="min. 6 characters" hint="You'll use this to log in" value={form.username} onChange={handleChange} error={errors.username} />
            <div className="form-group">
              <label className="form-label">Password *</label>
              <div style={{ position: 'relative' }}>
                <input className="form-input" type={showPass ? 'text' : 'password'} value={form.password} placeholder="min. 8 characters" onChange={e => handleChange('password', e.target.value)} />
                <button onClick={() => setShowPass(p => !p)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--slate)', cursor: 'pointer' }}>
                  {showPass ? <EyeOff size={16} /> : <Eye size={16} />}
                </button>
              </div>
              {errors.password && <div style={{ fontSize: 11, color: 'var(--danger)', display: 'flex', alignItems: 'center', gap: 4 }}><AlertCircle size={12} /> {errors.password}</div>}
            </div>
            <div className="form-group">
              <label className="form-label">Confirm Password *</label>
              <input className="form-input" type="password" value={form.confirmPassword} placeholder="Re-enter password" onChange={e => handleChange('confirmPassword', e.target.value)} />
              {errors.confirmPassword && <div style={{ fontSize: 11, color: 'var(--danger)', display: 'flex', alignItems: 'center', gap: 4 }}><AlertCircle size={12} /> {errors.confirmPassword}</div>}
            </div>
            <div style={{ background: 'var(--surface)', borderRadius: 10, padding: 16, marginTop: 8 }}>
              ...
              {[['8+ characters', form.password.length >= 8], ['One uppercase letter', /[A-Z]/.test(form.password)], ['One number', /[0-9]/.test(form.password)]].map(([label, ok]) => (
          ...
          {step === 4 && <>
            ...
            {[
              ...
              <div key={section} style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10, paddingBottom: 8, borderBottom: '1px solid var(--border)' }}>{section}</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 32 }}>
            <button onClick={() => step > 0 ? setStep(s => s - 1) : navigate('landing')} className="btn-outline">
              {step === 0 ? 'Cancel' : '← Back'}
            </button>
            {step < 4
              ? <button onClick={next} className="btn-primary">Continue <ChevronRight size={16} /></button>
              : <button onClick={submit} className="btn-gold" style={{ padding: '12px 32px' }}>Submit Application ✓</button>
            }
          </div>
        </div>
      </div>
